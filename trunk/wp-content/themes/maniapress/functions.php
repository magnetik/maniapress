...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="a"><xsl:text>$&lt;$l[</xsl:text><xsl:value-of select="@href"/><xsl:text>]</xsl:text><xsl:choose><xsl:when test="string-length(.)=0"><xsl:text>link</xsl:text></xsl:when></xsl:choose><xsl:apply-templates select="node()[not(self::img)]"/><xsl:text>$l$&gt;</xsl:text></xsl:template>
